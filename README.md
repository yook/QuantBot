...
- **Electron** — оболочка для десктопного приложения (main process на TypeScript).
- **Vue 3 + TypeScript + Vite** — frontend (renderer) приложения.
- **Element Plus** — UI-компоненты и стили для интерфейса.
- **SQLite (better-sqlite3)** — локальная синхронная база данных для хранения проектов, URL и кэшей.
- **Pinia** — состояние приложения (stores).
- **vue-i18n** — локализация.
- **node child_process / worker scripts** — тяжёлые задачи (краулер, кластеризация, классификация) выполняются отдельными Node-скриптами (CommonJS) как child processes.
...
3. Если хотите локально запустить dev: `npm run dev` (Vite + Electron). Режим `socket:dev` устарел — realtime через Socket.IO был удалён, запускать его не нужно.
