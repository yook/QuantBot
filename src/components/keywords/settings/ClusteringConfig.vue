        ...
        <el-collapse-item name="1">
          ...
          <div class="text-sm">
            <div class="mb-2">
              ...
              представление) с помощью OpenAI Embeddings. Далее DBSCAN
              группирует похожие запросы в кластеры, основываясь на плотности
              эмбеддингов в пространстве. В отличие от kNN или cosine
              similarity, DBSCAN не требует заранее заданных категорий и может
              обнаруживать кластеры произвольной формы, а также помечать выбросы
              (noise).
            </div>
            <div>
              <strong>Алгоритм:</strong><br />
              • Для каждого запроса вычислить embedding через OpenAI Embedding
              API.<br />
              • Собрать все эмбеддинги в матрицу (N x D, где N — количество
              запросов, D — размерность эмбеддинга).<br />
              • Применить алгоритм HDBSCAN к этой матрице эмбеддингов:<br />
              &nbsp;&nbsp;◦ Алгоритм ищет плотные области в пространстве
              эмбеддингов, соединяя точки, которые находятся ближе друг к другу,
              чем заданный порог (min_samples, min_cluster_size).<br />
              &nbsp;&nbsp;◦ Строится иерархия кластеров, далее выбираются
              наиболее стабильные кластеры.<br />
              &nbsp;&nbsp;◦ Точки, которые не принадлежат ни одному кластеру,
              считаются выбросами (noise).<br />
              • Для каждого запроса определяется метка кластера (или метка
              выброса).
            </div>
          </div>
        </el-collapse-item>
