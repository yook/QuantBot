...
function startSocketServer(port = undefined) {
  return new Promise((resolve, reject) => {
    ...
    const candidates = [8090]; // Установлен фиксированный порт 8090

    console.log(
      `Starting Socket.IO server in ${
        isDev ? "development" : "production"
      } mode; will try ports: ${candidates.join(", ")}`
    );
    ...
    const io = require("socket.io")(httpServer, {
      path: "/socket", // Изменен путь сокета на '/socket'
      cors: {
        origin: "*",
      },
    });
    ...
    const tryPortProbe = (ports) => {
      return new Promise((resolvePort, rejectPort) => {
        ...
        const tryNext = () => {
          if (idx >= ports.length)
            return rejectPort(
              new Error("No available ports to bind socket server")
            );
          const p = ports[idx++];
    ...
    tryPortProbe(Array.from(new Set(candidates)))
      .then((chosenPort) => {
